...
export interface IBanner extends Document {
  ...
  contentModel?: 'Content' | 'Movie' | 'Audio' | 'VideoMusic';
  type: 'hero' | 'featured' | 'promotional' | 'category';
  contentType: 'drama' | 'movie' | 'series' | 'audio' | 'video-music' | 'both';
  position: number;
  isActive: boolean;
  targetPlatforms: Array<'web' | 'mobile' | 'tv'>;
  targetPages?: Array<'home' | 'movies' | 'tvshows' | 'music' | 'videos'>;
  startDate?: Date;
  endDate?: Date;
  clickCount: number;
...
const BannerSchema = new Schema<IBanner>(
  {
    ...
    ctaLink: String,
    contentId: { type: Schema.Types.ObjectId, refPath: 'contentModel' },
