...
const findContent = async (contentId: string, contentType: string) => {
  ...
  if (contentType === 'audio') {
    return AudioModel.findById(contentId).select('likes').lean();
  }
  if (contentType === 'videoMusic') {
    return VideoMusicModel.findById(contentId).select('likes').lean();
  }
...
const updateLikes = async (contentId: string, contentType: string, increment: 1 | -1) => {
  if (contentType === 'movie') {
    return MovieModel.findByIdAndUpdate(contentId, { $inc: { likes: increment } }, { returnDocument: 'after' }).select('likes').lean();
  }
  if (contentType === 'audio') {
    return AudioModel.findByIdAndUpdate(contentId, { $inc: { likes: increment } }, { returnDocument: 'after' }).select('likes').lean();
  }
  if (contentType === 'videoMusic') {
    return VideoMusicModel.findByIdAndUpdate(contentId, { $inc: { likes: increment } }, { returnDocument: 'after' }).select('likes').lean();
  }
  return ContentModel.findByIdAndUpdate(contentId, { $inc: { likes: increment } }, { returnDocument: 'after' }).select('likes').lean();
};
...
export const toggleLike = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const body = request.body as { contentType?: 'drama' | 'movie' | 'audio' | 'videoMusic'; episodeId?: string };
    const contentType = body?.contentType || 'drama';
    const contentModelType = contentType === 'movie' ? 'Movie' : contentType === 'audio' ? 'Audio' : contentType === 'videoMusic' ? 'VideoMusic' : 'Content';
