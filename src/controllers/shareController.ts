...
const APP_PACKAGE_NAME = process.env.APP_PACKAGE_NAME || 'com.brajcinema.tv';
const APP_SCHEME = process.env.APP_SCHEME || 'brajcinema';
...
const incrementShareCount = async (contentId: string, contentType?: string) => {
  try {
    if (contentType === 'movie') {
      ...
      await AudioModel.findByIdAndUpdate(contentId, { $inc: { shares: 1 } });
      return;
    } else if (contentType === 'videoMusic') {
      await VideoMusicModel.findByIdAndUpdate(contentId, { $inc: { shares: 1 } });
      return;
    ...
    const movieUpdated = await MovieModel.findByIdAndUpdate(contentId, { $inc: { shares: 1 } });
    if (!movieUpdated) {
      await ContentModel.findByIdAndUpdate(contentId, { $inc: { shares: 1 } });
    }
  } catch (err) {
...
export const recordShare = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const { contentId } = request.params as { contentId: string };
    const body = request.body as { contentType?: 'drama' | 'movie' | 'series' | 'tv-show' | 'audio' | 'videoMusic' } || {};
    ...
    if (contentType === 'movie') {
      ...
      const audio = await AudioModel.findById(contentId).select('shares').lean();
      sharesCount = audio?.shares ?? 0;
    } else if (contentType === 'videoMusic') {
      const video = await VideoMusicModel.findById(contentId).select('shares').lean();
    ...
    return reply.status(500).send({ success: false, message: 'Failed to record share.', error: error.message });
  }
};
