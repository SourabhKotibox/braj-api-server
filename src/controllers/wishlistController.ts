...
export const toggleWishlist = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const isVideoMusic = rawType === 'videoMusic';
    const contentModelType = isMovie ? 'Movie' : isAudio ? 'Audio' : isVideoMusic ? 'VideoMusic' : 'Content';
    ...
    let content;
    if (isMovie) {
      content = await MovieModel.findById(contentId).select('_id');
    } else if (isAudio) {
      content = await AudioModel.findById(contentId).select('_id');
    } else if (isVideoMusic) {
      content = await VideoMusicModel.findById(contentId).select('_id');
    } else {
      content = await ContentModel.findById(contentId).select('_id');
    }
    if (!content) {
