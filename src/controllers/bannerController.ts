...
const populateBannersContent = async (banners: any[]) => {
  ...
  const [movies, contents, audios, videoMusics] = await Promise.all([
    ...
    AudioModel.find({ _id: { $in: contentIds } }).lean(),
    VideoMusicModel.find({ _id: { $in: contentIds } }).lean(),
  ]);

  // Create a map for quick lookups
  const contentMap = new Map();
  for (const movie of movies) {
    contentMap.set(movie._id.toString(), { ...movie, contentType: 'movie', contentModel: 'Movie' });
  }
  for (const content of contents) {
    contentMap.set(content._id.toString(), { ...content, contentType: content.contentType || 'series', contentModel: 'Content' });
  }
  for (const audio of audios) {
    contentMap.set(audio._id.toString(), { ...audio, contentType: 'audio', contentModel: 'Audio' });
  }
  for (const videoMusic of videoMusics) {
    contentMap.set(videoMusic._id.toString(), { ...videoMusic, contentType: 'video-music', contentModel: 'VideoMusic' });
  }

  // Assign populated content back to banner
  for (const banner of banners) {
    if (banner.contentId) {
      const content = contentMap.get(banner.contentId.toString());
      banner.contentId = content || null;
      banner.contentModel = content?.contentModel || banner.contentModel;
    }
  }
...
export const createBannerFromContent = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const body = request.body as {
      contentId: string;
      contentSource: 'movie' | 'content' | 'audio' | 'video-music';
      title?: string;
      subtitle?: string;
      description?: string;
      ctaText?: string;
    ...
    if (body.contentSource === 'movie') {
      ...
      contentType = 'movie';
      contentModel = 'Movie';
    } else if (body.contentSource === 'audio') {
      source = await AudioModel.findById(body.contentId).lean();
      contentType = 'audio';
      contentModel = 'Audio';
    } else if (body.contentSource === 'video-music') {
      source = await VideoMusicModel.findById(body.contentId).lean();
    ...
    const thumbnail = source.thumbnail || source.bannerImage || source.coverImage || source.imageUrl || ensureDefaultBannerImage();
