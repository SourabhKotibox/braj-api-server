...
const ROLE_HIERARCHY = ['influencer', 'moderator', 'admin', 'super_admin', 'superadmin'] as const;
...
export const getRoleRank = (role: string): number => {
  ...
  if (role === 'superadmin' || role === 'super_admin') return ROLE_HIERARCHY.length - 1;
  return idx;
};
...
export const checkUserPermission = async (
  ...
  if (user.role === 'superadmin' || user.role === 'super_admin') return { allowed: true, user };
  ...
  if (!action) {
    const modulePermissions = (user.modulePermissions as any)?.[moduleName] || (user.permissions as any)?.[moduleName];
  ...
  // Check specific action permission (handle both modulePermissions and permissions)
  const modulePermissions = (user.modulePermissions as any)?.[moduleName] || (user.permissions as any)?.[moduleName];
 ...
 */
export const requireAdmin = requireRole('admin', 'super_admin', 'superadmin');
